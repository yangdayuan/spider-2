...
//            setVia(null);
   ...
   /**
    ...
    throws URIException {
    	/**
    	 * 302重定向不使用重定向前的链接作为via
    	 * @modify: wuliufu
    	 * @since : 2012-05-11
    	 */
        UURI u = null;
        CandidateURI newCaURI = null;
            if(link.getDestination() instanceof UURI) {
            	u = (UURI)link.getDestination();
            	UURI via = null;
            	if(link.getSource() != null) {
            		via = link.getSource() instanceof UURI ? (UURI)link.getSource() :UURIFactory.getInstance(link.getSource().toString());
            	} else {
            		via = u;
            	}
            	newCaURI = new CandidateURI(u, getPathFromSeed() + link.getHopType(),
            			via, link.getContext());
            } else {
            	u = UURIFactory.getInstance(baseUURI,
