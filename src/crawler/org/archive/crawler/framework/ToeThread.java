...
Reporter, ProgressStatisticsReporter {
    ...
    private HashMap<String,Processor> localProcessors = new HashMap<String,Processor>();
    ...
    public void run() {
        ...
        logger.debug(getName()+" started for order '"+name+"'");
        ...
        try {
            ...
            while ( true ) {
                ...
                synchronized(this) {
    		    	...
    		    	if(currentCuri != null && currentCuri.getHttpRecorder() != null) {
    		    		try {
    		    			currentCuri.getHttpRecorder().closeRecorders();
    		                logger.debug("cloase backup file.&uri= " + currentCuri.getCrawlURIString());
    		    		} catch(Throwable t) {
    		    			logger.error("Close backup fileStream error.&uri= " + currentCuri.getCrawlURIString(), t);
    		    		}
    		    	}
                    setCurrentCuri(null);
                }
        ...
        logger.debug(getName() + " finished for order '" + name + "'");
	...
	private void seriousError(Error err) {
        ...
        if (controller!=null) {
            ...
            controller.releaseSingleThreadMode();
        }
